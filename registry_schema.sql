...
namespace NyxKernel {
    public static class RegistryManager {
        ...
        static RegistryManager() {
            // Using your specific, robust schema definition
            string schema = @"
        ...
        public static void GraftSkill(string name, string alias, string logic, string source) {
            lock (_dbLock) {
                using (var conn = new SQLiteConnection(dbPath)) {
                    conn.Open();
                    // Using OR REPLACE to leverage your UNIQUE command_alias constraint
                    string sql = @"INSERT OR REPLACE INTO skills (name, command_alias, logic_block, source_agent, last_accessed) 
                                   VALUES (@n, @a, @l, @s, CURRENT_TIMESTAMP)";
